...
# as Jmax=4 and eps=1e-6 makes no sense, skip it. just the smallest 1e-7 value is contained!
EPS_max=( 5 10 15 20 )
...
do
	...
	do
		...
		dir=${pre}_halfswirl_Bs${bs}_i${q}_Jmax${JMAX[$a]}_eps${EPS[$j]}
		mkdir $dir
		echo $dir
		cd $dir
		cp ../$ini .
		...
		$mpi ./wabbit 2D $ini --memory=10.0GB
		cd ..
		q=$((q+1))
	done
done
