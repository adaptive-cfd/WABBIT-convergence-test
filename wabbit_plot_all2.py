...
#for prefix, cmap in zip(['mask'],['jet']):
    files = glob.glob('/home/engels/dev/WABBIT4-new-physics/three-vortices/eps1e-4/'+prefix+'_*.h5')
    files.sort()

    if cmap is 'jet':
        fc = 'jet'
    else:
        fc = farge_colormaps.farge_colormap_multi( taille=600, limite_faible_fort=0.2, etalement_du_zero=0.04, type=cmap )
    ...
    for file, i in zip(files[1:], range(len(files[1:]))):
        wabbit_tools.plot_wabbit_file( file, savepng=True, cmap=fc, ticks=False, colorbar=True, dpi=300, title=True,
                                      block_edge_color='w', caxis_symmetric=True, shading='gouraud' )
